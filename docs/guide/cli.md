...
`@jalsoedesign/dockline-cli` provides the `dockline` command for downloading, uploading, listing and removing remote files. Server settings live in YAML; everyday file choices stay on the command line. Install only the FTP or SFTP client you need.
